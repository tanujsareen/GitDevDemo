...
List Repository for all API Call / Database Call
 **/
class GitRepository {

    /*call Developer API : parameters(language=["Java, Kotlin"], since="weekly")*/
    fun callDevelopersAPI(
        language: String,
        since: String
    ): MutableLiveData<Outcome<ArrayList<GitItem>>> {
     ...
     /*call*/
        BaseNetwork.callAPI.callDevelopers(language, since)
            .enqueue(object : Callback<ArrayList<GitItem>> {
                ...
                override fun onResponse(
                    call: Call<ArrayList<GitItem>>,
                    response: Response<ArrayList<GitItem>>
                ) {
